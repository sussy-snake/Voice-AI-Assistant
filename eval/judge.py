...
def _resolve_provider() -> str:
    ...
    if forced in ("anthropic", "auto") and (os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN")):
        return "anthropic"
    if forced in ("gemini", "auto") and (os.environ.get("GEMINI_API_KEY") or os.environ.get("GOOGLE_API_KEY")):
        return "gemini"
    ...
    raise JudgeNotConfigured(
        ...
        "  GROQ_API_KEY        (Groq Cloud Llama 3.1)\n"
...
def _parse_verdict(raw: str) -> tuple[bool, str]:
    try:
        ...
        # Fallback keyword match if model didn't wrap cleanly
        verdict = ("true" in raw.lower()) and ("false" not in raw.lower())
        return verdict, f"[judge raw: {raw[:200]!r}]"
...
def _call_gemini(system_prompt: str, user_content: str) -> JudgeVerdict:
    ...
    full_prompt = (
        f"{system_prompt}\n\n"
        f"Strict JSON Contract: You must respond ONLY with a valid JSON object matching {{\"verdict\": boolean, \"reason\": string}}.\n\n"
        f"{user_content}"
    )
    payload = json.dumps({
    ...
    with urllib.request.urlopen(req, timeout=10) as resp:
        ...
        return JudgeVerdict(verdict=verdict, reason=reason, judge_ms=(time.perf_counter() - t0) * 1000, provider="gemini", raw=raw)
...
def _call_groq(system_prompt: str, user_content: str) -> JudgeVerdict:
    ...
    with urllib.request.urlopen(req, timeout=10) as resp:
        ...
        verdict, reason = _parse_verdict(raw)
        return JudgeVerdict(verdict=verdict, reason=reason, judge_ms=(time.perf_counter() - t0) * 1000, provider="groq", raw=raw)
